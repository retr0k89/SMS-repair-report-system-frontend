...
import { Select, Button, Space, Table, Tag, Breadcrumb, Modal, Collapse, Segmented, Input } from 'antd';
import axios from "axios";

const DashBoard = () => {
    const [fillinUnit, setFillingUnit] = React.useState("");
    const [fixingUnit, setFixingUnit] = React.useState("");
    const [repairItem, setRepairItem] = React.useState("");
    const [awaitProcess, setAwaitProcess] = React.useState("尚待處理");
    ...
    const [detailModalStatus, setDetailModalStatus] = React.useState(false);
    const [replyModalStatus, setReplyModalStatus] = React.useState(false);
    const [choosenReplyValue, setChoosenReplyValue] = React.useState("");
    const [replyContent, setReplyContent] = React.useState("");
    const [updateFixReportData, setUpdateFixReportData] = React.useState({
        number: "",
        fillinUnit: "",
        notifyUnit: "",
        repairItem: "",
    ...
    const [choosenReplyStatus, setChoosenReplyStatus] = React.useState("");
    ...
    const fetchData = () => {
        var dataList = []
        axios.post("http://127.0.0.1:8000/welcome/api/fixReport", {
            unit: fillinUnit,
            item: fixingUnit,
            notice: repairItem,
            status: awaitProcess
    ...
    const reply = (e, value) => {
        axios.post("http://127.0.0.1:8000/welcome/api/fetchFixReport", {
            id: value.number
        })
        .then(res => {
    ...
    const viewDetail = (e, value) => {
        axios.post("http://127.0.0.1:8000/welcome/api/viewFixReportDetail", {
            id: value.number
        })
        .then(res => {
            console.log(res)
    ...
    const updateData = (dataPack) => {
        axios.post("http://127.0.0.1:8000/welcome/api/updateFixReport", {
